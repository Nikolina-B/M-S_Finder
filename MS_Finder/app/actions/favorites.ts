 ...
 */
export async function toggleFavoritesAction(movie: {
  imdbID: string;
  Title: string;
  imdbRating: string;
  Year: string;
  ...
  if (!session || !session.user) {
    throw new Error("Morate biti prijavljeni kako biste uredili wishlistu.");
  }
  ...
  try {
    ...
    const existingEntry = await db
      ...
      .where(
        and(
          ...
          eq(favorites.movieId, movie.imdbID)
        )
      )
      .limit(1);
    ...
    console.error("Greška u wishlist akciji:", error);
