...
export default function Navbar() {
  ...
  const [openMenu, setOpenMenu] = useState<MenuName>(null);
  const [userImage, setUserImage] = useState<string | null>(null); // State za profilnu sliku
  const router = useRouter();
  ...
  // Funkcija za dohvaćanje inicijala ako nema slike
  const getInitials = (name: string) => {
    return name
      .split(" ")
      .map((n) => n[0])
  ...
  // Učitavanje slike iz localStorage i slušanje promjena
  useEffect(() => {
    const loadLocalAvatar = () => {
      ...
      if (stored) {
        try {
          ...
          console.error("Greška pri učitavanju avatara:", e);
        }
      }
    };
    ...
    // Slušaj promjene iz EditProfile komponente
    window.addEventListener("storage", loadLocalAvatar);
    window.addEventListener("profileUpdated", loadLocalAvatar);

    return () => {
      window.removeEventListener("storage", loadLocalAvatar);
      window.removeEventListener("profileUpdated", loadLocalAvatar);
    };
  }, []);
